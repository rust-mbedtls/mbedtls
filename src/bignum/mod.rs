...
impl Mpi {
    ...
    pub fn new(value: mbedtls_mpi_sint) -> Result<Mpi> {
        let mut ret = Self::init();
    ...
    pub fn divrem(&self, other: &Mpi) -> Result<(Mpi, Mpi)> {
        let mut q = Self::init();
        let mut r = Self::init();
        unsafe { mbedtls_mpi_div_mpi(&mut q.inner, &mut r.inner, &self.inner, &other.inner) }
            .into_result()?;
        Ok((q, r))
    }
    ...
    pub fn divrem_int(&self, other: mbedtls_mpi_sint) -> Result<(Mpi, Mpi)> {
        let mut q = Self::init();
        let mut r = Self::init();
        unsafe { mbedtls_mpi_div_int(&mut q.inner, &mut r.inner, &self.inner, other) }.into_result()?;
        Ok((q, r))
    }
...
macro_rules! impl_arithmetic_op {
    ($op_trait:ident, $op_assign_trait:ident, $trait_func:ident, $trait_assign_func:ident, $func:expr, $int_func:expr) => {
        ...
        impl<'a> $op_trait<mbedtls_mpi_sint> for &'a Mpi {
            type Output = Result<Mpi>;

            fn $trait_func(self, other: mbedtls_mpi_sint) -> Result<Mpi> {
                let mut ret = Mpi::init();
        ...
        impl $op_assign_trait<mbedtls_mpi_sint> for Mpi {
            fn $trait_assign_func(&mut self, other: mbedtls_mpi_sint) {
                unsafe { $int_func(self.handle_mut(), self.handle(), other as _) }
                    .into_result()
                    .expect("mbedtls_mpi_add_int worked");
            }
        }
    };
}
...
impl<'a> Div<mbedtls_mpi_sint> for &'a Mpi {
    type Output = Result<Mpi>;

    fn div(self, other: mbedtls_mpi_sint) -> Result<Mpi> {
        let mut q = Mpi::init();
        unsafe { mbedtls_mpi_div_int(&mut q.inner, ::core::ptr::null_mut(), &self.inner, other) }
            .into_result()?;
        Ok(q)
    }
}
...
impl<'a> DivAssign<&'a Mpi> for Mpi {
    fn div_assign(&mut self, other: &Mpi) {
        // mbedtls_mpi_div_mpi produces incorrect output when arguments alias, so avoid doing that
        let mut q = Mpi::init();
        unsafe {
            mbedtls_mpi_div_mpi(
                &mut q.inner,
        ...
        .into_result()
        .expect("mbedtls_mpi_div_mpi success");
        *self = q;
    }
}
...
/// Note this will panic if other == 0
impl DivAssign<mbedtls_mpi_sint> for Mpi {
    fn div_assign(&mut self, other: mbedtls_mpi_sint) {
        unsafe {
            mbedtls_mpi_div_int(
                self.handle() as *const mbedtls_mpi as _,
                ::core::ptr::null_mut(),
                &self.inner,
                other,
            )
        ...
        .into_result()
        .expect("mbedtls_mpi_div_int success");
    }
}
...
impl Rem<mbedtls_mpi_sint> for Mpi {
    type Output = Result<Mpi>;

    fn rem(self, other: mbedtls_mpi_sint) -> Result<Mpi> {
        let mut r = Mpi::init();
        unsafe { mbedtls_mpi_div_int(::core::ptr::null_mut(), &mut r.inner, &self.inner, other) }
            .into_result()?;
        Ok(r)
    }
}

impl<'a> Rem<mbedtls_mpi_sint> for &'a Mpi {
    type Output = Result<Mpi>;
...
/// Note this will panic if other == 0
impl RemAssign<mbedtls_mpi_sint> for Mpi {
    fn rem_assign(&mut self, other: mbedtls_mpi_sint) {
        unsafe {
            mbedtls_mpi_div_int(
                ::core::ptr::null_mut(),
                self.handle() as *const mbedtls_mpi as _,
...
impl<'a> Shl<usize> for &'a Mpi {
    ...
    fn shl(self, shift: usize) -> Result<Mpi> {
        ...
        unsafe { mbedtls_mpi_shift_l(&mut r.inner, shift) }.into_result()?;
        Ok(r)
    }
}
...
impl ShlAssign<usize> for Mpi {
    fn shl_assign(&mut self, shift: usize) {
        unsafe { mbedtls_mpi_shift_l(self.handle() as *const mbedtls_mpi as _, shift) }
            .into_result()
            .expect("mbedtls_mpi_shift_l success");
    }
}
...
impl<'a> Shr<usize> for &'a Mpi {
    ...
    fn shr(self, shift: usize) -> Result<Mpi> {
        let mut r = Mpi::copy(self.handle())?;
        unsafe { mbedtls_mpi_shift_r(&mut r.inner, shift) }.into_result()?;
        Ok(r)
    }
}
...
impl ShrAssign<usize> for Mpi {
    fn shr_assign(&mut self, shift: usize) {
        unsafe { mbedtls_mpi_shift_r(self.handle() as *const mbedtls_mpi as _, shift) }
            .into_result()
            .expect("mbedtls_mpi_shift_l success");
    }
}
...
// TODO
// mbedtls_mbedtls_mpi_safe_cond_assign
// mbedtls_mbedtls_mpi_safe_cond_swap
// mbedtls_mbedtls_mpi_lset
// mbedtls_mbedtls_mpi_lsb
// mbedtls_mbedtls_mpi_write_string
// mbedtls_mbedtls_mpi_cmp_abs
// mbedtls_mbedtls_mpi_cmp_int
// mbedtls_mbedtls_mpi_add_abs
// mbedtls_mbedtls_mpi_sub_abs
// mbedtls_mbedtls_mpi_mod_int
// mbedtls_mbedtls_mpi_gcd
// mbedtls_mbedtls_mpi_is_prime
